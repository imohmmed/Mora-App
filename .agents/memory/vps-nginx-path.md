...
Admin sends `Authorization: Bearer dev-token-mora` (setAuthTokenGetter in `artifacts/admin/src/main.tsx`, AUTH_HEADER in `content/index.tsx`); requireAdmin checks that exact string. It ships inside the JS served at `admin.moramoda.tech`, so anyone can extract it and write/delete now-persistent prod data. **Until T007 (admin_users + bcrypt + sessions) lands, production has an open write hole.** Also: VPS root password is still in git history (earlier commit) — rotation is the only fix.
