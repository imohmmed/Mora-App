...
router.post("/admin/products", (req, res) => {
  ...
  db.prepare(`INSERT INTO products (id,title,vendor,category,gender,description,price,compare_price,cost,images,tags,status,option_definitions,seo_title,seo_description,url_slug,rating,rating_count,video_url,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
    .run(id, b["title"] ?? "", b["vendor"] ?? "", b["category"] ?? "women", b["gender"] ?? "all", b["description"] ?? "", b["price"] ?? 0, b["compareAtPrice"] ?? null, b["cost"] ?? null, JSON.stringify(b["images"] ?? []), JSON.stringify(b["tags"] ?? []), b["status"] ?? "draft", JSON.stringify(b["optionDefinitions"] ?? []), b["seoTitle"] ?? "", b["seoDescription"] ?? "", slug, b["rating"] ?? 0, b["ratingCount"] ?? 0, (b["videoUrl"] as string) ?? "", now, now);
...
router.put("/admin/products/:id", (req, res) => {
  ...
  db.prepare(`UPDATE products SET title=COALESCE(?,title), vendor=COALESCE(?,vendor), category=COALESCE(?,category), gender=COALESCE(?,gender), description=COALESCE(?,description), price=COALESCE(?,price), compare_price=?, cost=COALESCE(?,cost), images=COALESCE(?,images), tags=COALESCE(?,tags), status=COALESCE(?,status), option_definitions=COALESCE(?,option_definitions), seo_title=COALESCE(?,seo_title), seo_description=COALESCE(?,seo_description), url_slug=COALESCE(?,url_slug), rating=COALESCE(?,rating), rating_count=COALESCE(?,rating_count), video_url=COALESCE(?,video_url), updated_at=? WHERE id=?`)
    .run(b["title"] ?? null, b["vendor"] ?? null, b["category"] ?? null, b["gender"] ?? null, b["description"] ?? null, b["price"] ?? null, b["compareAtPrice"] ?? null, b["cost"] ?? null, b["images"] !== undefined ? JSON.stringify(b["images"]) : null, b["tags"] !== undefined ? JSON.stringify(b["tags"]) : null, b["status"] ?? null, b["optionDefinitions"] !== undefined ? JSON.stringify(b["optionDefinitions"]) : null, b["seoTitle"] ?? null, b["seoDescription"] ?? null, b["urlSlug"] ?? null, b["rating"] !== undefined ? b["rating"] : null, b["ratingCount"] !== undefined ? b["ratingCount"] : null, b["videoUrl"] !== undefined ? (b["videoUrl"] as string) : null, now, id);
